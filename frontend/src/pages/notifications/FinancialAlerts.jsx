...
export default function FinancialAlerts() {
  ...
  const fetchNotifications = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await notificationService.getAll({ category: 'financial' });
      if (!isMounted.current) return;
      const { notifications: data } = res.data;
      setNotifications((data || []).map(n => ({ ...n, id: n._id, description: n.message || n.title, read: n.isRead, date: n.createdAt, category: 'financial' })));
    } catch (err) {
      if (isMounted.current) setError(err.response?.data?.message || err.message || 'Failed to fetch financial alerts');
    } finally {
  ...
  const overdue = notifications.filter((n) => n.subcategory === "Invoice Overdue").length;
  const budgetExceeded = notifications.filter((n) => n.subcategory === "Budget Exceeded").length;
  const expenseApproved = notifications.filter((n) => n.subcategory === "Expense Approved").length;

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <PageHeader title="Financial Alerts" subtitle={`${notifications.length} financial notifications`}
        badge={`${overdue} Overdue`}
      />
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
        <StatCard label="Total Financial Alerts" value={notifications.length} icon={DollarSign} color="success" />
        ...
        <StatCard label="Expense Approved" value={expenseApproved} icon={DollarSign} color="primary" />
      </div>
      {loading ? (
        <LoadingSkeleton />
      ) : error ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <h3 className="text-base font-bold text-neutral-textMain mb-1">Failed to load financial alerts</h3>
