...
import {
  dockData, warehouseWorkers, warehouseSections, warehouseCapacityHistory,
  dockUtilizationHistory, workerProductivity, storageDistribution,
  incomingOutgoing, incomingDeliveries,
...
export default function WarehousePage() {
  ...
  const occupancyPct = Math.round((totalUsed / totalCapacity) * 100);
  const activeDocks = dockData.filter(d => d.status === 'Occupied').length;
  const availableDocks = dockData.filter(d => d.status === 'Available').length;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8 gap-3 mb-5">
        ...
        <StatCard title="Dock Status" value={`${activeDocks}/${dockData.length}`} icon={Dock} change="+3" type="up" sparklineData={[12, 14, 15, 13, 16, 17, activeDocks]} color="bg-blue-50 text-blue-600" delay={0.06} />
        <StatCard title="Available Docks" value={availableDocks} icon={Loader} change="-2" type="down" sparklineData={[8, 7, 6, 5, 4, 3, availableDocks]} color="bg-cyan-50 text-cyan-600" delay={0.08} />
      ...
      <div className="bg-white border border-neutral-border rounded-xl p-4 shadow-soft-sm mb-5">
        ...
        <div className="w-full h-3 bg-neutral-light rounded-full overflow-hidden">
          <motion.div initial={{ width: 0 }} animate={{ width: `${occupancyPct}%` }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
        ...
        <ChartCard title="Dock Usage" subtitle="Current dock utilization" delay={0.25}
          actions={<TrendIndicator value={`${Math.round(dockData.reduce((s, d) => s + d.utilization, 0) / dockData.length)}% avg`} type="neutral" />}>
        ...
        <ChartCard title="Storage Distribution" subtitle="By section type" delay={0.3}>
          <DonutChart data={storageDistribution} size={130} thickness={15} />
        </ChartCard>
      </div>
      ...
      {/* Dock Status Grid */}
      <ChartCard title="Dock Status Overview" subtitle={`${dockData.length} docks - ${activeDocks} active`} delay={0.45}
